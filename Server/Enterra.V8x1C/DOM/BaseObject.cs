...
namespace Enterra.V8x1C.DOM
{
    ...
    public abstract class BaseObject : IDisposable
    {
        ...
        internal static object InvokeV8Method(object target, string name, object[] args)
        {
            ...
            //{
            //    throw new Exception(
            //        string.Format("Ошибка при обращении к COM+: InvokeV8Method failed: target={0}; name={1}; args=[{2}]: {3}",
            //        target.GetHashCode(), name, ArgsToStr(args), ex.Message), ex);
            //}
        ...
        /// <summary>
        /// Задержка для "отдыха" ком+-компоненты
        /// </summary>
        /// <remarks>опытным путем установлено, что если в циклах, в которых идет интенсивное обращение
        /// к ком+, вставить задержку, то ком+ работает стабильнее и падает намного реже</remarks>
        public static void RestDelay()
        {
            GC.Collect();
            Thread.Sleep(20);
        }

        private static string ArgsToStr(ICollection<object> args)
        {
            if (args == null)
                return "<null>";
